...
async function runtime(id, seasons, runtime) {
  ...
  for(let i = 1; i <= seasons; i++) {
    ...
    buttons += `<span onclick="updateRuntime('${runtime}', '${ep}', this)">${i}</span> `;
    runtime = d = h = m = 0;
  }
  ...
  return `<h2 style="text-align: center;">Seasons: <span id="total" onclick="updateRuntime('${total}', '${totalEp}', this)">All</span> ${buttons}</h2><h1 id="runtime"><h1>`
}
